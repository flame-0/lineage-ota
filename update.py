        ...
        with zipfile.ZipFile(io.BytesIO(file_content)) as archive:
            ...
            for line in metadata.splitlines():
                if line.startswith(b"post-timestamp="):
                    datetime = int(line.split(b"=", 1)[1])
                elif line.startswith(b"post-security-patch-level="):
                    os_patch_level = line.split(b"=", 1)[1].decode()
                elif line.startswith(b"post-sdk-level="):
                    os_sdk_level = int(line.split(b"=", 1)[1])
    except KeyError:
        raise ValueError(
            "META-INF/com/android/metadata is missing from the zip; "
            "the updater will not accept an update without it"
        )
    if datetime is None or os_patch_level is None or os_sdk_level is None:
        raise ValueError(
            "metadata is missing post-timestamp, post-security-patch-level, "
            "or post-sdk-level"
        )
